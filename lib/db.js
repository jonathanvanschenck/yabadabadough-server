...
module.exports = {
    create_connection: function(config) {
        ...
        if ( !path ) {
            throw new Error(
                "No database path configured: set YDD_SQLITE_PATH "
                + "(':memory:' for an ephemeral database)"
            );
        }

        if ( path != ":memory:" ) {
            // Absolute paths are used verbatim. Relative ones resolve against
            // the APP ROOT, not process.cwd(), so `node index.js` and
            // `node scripts/create-user.js` open the same file no matter which
            // directory they were invoked from.
            path = isAbsolute(path) ? path : join(__dirname, "..", path);
